...
// 1_workerPool_semafore. Как это работает, что не так, что поправить?
// ===========================================================
func main() {
	//обзор проблем:

	//ch := make(chan bool)
	//ch <- true //1_workerPool_semafore.после этого будет deadlock ,т.к.нужно будет прочитать из не буф.канала после записи
	//go func() {
	//	<-ch
	...
	ch := make(chan bool)
	wg := sync.WaitGroup{}
	//ch <- true//1_workerPool_semafore.перенесена после горутины чтения
	wg.Add(1) //2.добавить 1_workerPool_semafore горутину
	go func() {
	...
	ch <- true //1_workerPool_semafore.вот сюда
	ch <- true
...
//===========================================================
//Задача 2
//1_workerPool_semafore. Как будет работать код?
...
//===========================================================
//Задача 4
//1_workerPool_semafore. Что выведет код и как исправить?
//===========================================================
//
